...
public class TeleopController extends Controller {
    ...
    public void handle() {
        ...
        else if(codriver.getTriggerAxis(Hand.kLeft) > Constants.XBOX_DEADBAND) Arm.getInstance().spit();
        else if(codriver.getTriggerAxis(Hand.kRight) > Constants.XBOX_DEADBAND) Arm.getInstance().hold();
        else if(codriver.getBumper(Hand.kLeft)) Arm.getInstance().drop();
        else Arm.getInstance().stopIntake();


    }

    public boolean getRotateAuto() {
